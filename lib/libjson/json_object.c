...
int json_object_object_exist(struct json_object* jso, const char *key)
{
	if(lh_table_lookup(jso->o.c_object, key) == NULL) return 0;
	return 1;
}						  
...
int json_object_get_int(struct json_object *jso)
{
  ...
  case json_type_boolean:
    return jso->o.c_boolean;
  case json_type_string:
    if(sscanf(jso->o.c_string.str, "%d", &cint) == 1) return cint;
  default:
    return 0;
  }
}
...
char* get_json_string_val(json_object* json_obj, const char* key) {
  return mem_str_cpy(json_object_get_string(json_object_object_get(json_obj, key)));
}

char* mem_str_cpy(const char* in) {
  char * out;
